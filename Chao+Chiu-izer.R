...
###############################################################################
chaochiuizer(Z=data1985,q=2,output = "everything")
...
chaochiuizer(Z=data1985,q=0,output = "normdiv1")
chaochiuizer(Z=data1985,q=0.5,output = "normdiv1")
chaochiuizer(Z=data1985,q=1,output = "normdiv1")
chaochiuizer(Z=data1985,q=1.5,output = "normdiv1")
...
for (i in seq(qute)) {
  normvector1985[i]<- chaochiuizer(Z=data1985, output= "normdiv1", q=qute[i])
  normvector1983[i]<- chaochiuizer(Z=data1983,output= "normdiv1",q=qute[i])
}

nonormvector1983<-NULL
nonormvector1985<-NULL
for (i in seq(qute)) {
  nonormvector1985[i]<- chaochiuizer(Z=data1985, output= "qdiv", q=qute[i])
  nonormvector1983[i]<- chaochiuizer(Z=data1983,output= "qdiv",q=qute[i])
}
...
#figure 2a
nonnormalized_divergence<-data.frame(qute,nonormvector1983,nonormvector1985)%>%
  pivot_longer(2:3,names_to = "year",values_to = "differentation")%>%
  ggplot(aes(x=qute, y=differentation, group=year, color=year))+geom_line()+geom_point()
nonnormalized_divergence